...
公共插件目录插图轮更新：首屏为薄荷色双栏展台，左侧标题/搜索、右侧专属工具箱插图；手机插图缩至180×135并置顶，分类以两列排列，为英文标签和真实数量留足空间。浅色展台 token 为 `--catalog-showcase: #e6f0e9`、`--catalog-ink: #243c30`、`--catalog-secondary: #53685b`；深色对应为 `#23372e`、`#e4f2e9`、`#b4cbbd`，跟随系统复用全局已解析主题，搜索及列表跟随主题。插图在两种主题均保持原始亮度，不加整体滤镜。卡片显示名称前两个字符的装饰字标（19px）与类型图标；摘要两行，详情保留完整内容。素材 `workshop-marketplace.webp` 为1000×750 RGBA透明插图；基于原始ImageGen图经用户授权本地Python连通抠底与边缘去底色，保留角色、眼睛高光和图标，大小215644 bytes。此条替代此前目录居中无插图设计。

公共插件目录使用上述双栏展台、带真实数量的类型导航、桌面三列/平板两列/手机单列卡片。类型图标用薄荷/黄色/中性 token，展示名称、slug、两行摘要与版本/托管元信息；整卡进入详情。每页 12 项，桌面数字分页、手机前后页与当前页数，URL 保留筛选与页码。此条仅更新公共目录，不改变其他数据列表页尺寸规范。

公共目录字号：主标题 32–52px 流式，结果标题 20px，卡片标题 17px，搜索 15px（手机 16px），摘要 14px，范围提示 13px，类型/slug/版本 12px。
...
原生桌面工作台延续现有工坊体系，具体布局以 `desktop/ui/src/workbench.css` 为准：244px 固定宽度侧栏、72px 顶栏和最大 1440px 主内容；1100px 以下侧栏缩至 216px、概览改单栏，760px 以下改为顶部四列导航。主标题 28px、窄窗口 24px；装备记录的类型、版本、客户端与操作分层，日志窄窗口纵排，诊断结果保留文字状态。此为代码提取的桌面表面规范，不改变下述 Web 布局；未据此宣称完成 GUI 视觉验收。

桌面 900px 起为 264px 侧栏，收起后 80px；侧栏导航独立滚动。主内容最大 1360px 居中，避免 2560px 大屏把信息拉散。900px 以下使用可展开导航。
...
- 原生工作台使用带图标与文字的四页导航，当前页有黄色底与 `aria-current`，切页后焦点移至标题；操作进行中禁用导航。概览保留缩小的专属桌面插画，装备/日志/诊断以真实记录与状态为主。类型图标配文字区分 MCP 与 Skill；卸载确认就近展开，危险操作明确标注。
- 我的装备以独立“PluginPocket MCP 网关”区块呈现服务端工具与 bridge 接入客户端，并提供返回概览的管理动作；本地直连 MCP/Skill 保留逐行管理。服务端托管标签说明运行位置，避免把远程工具画成本地安装文件；目录加载、不可用与重试保留可读状态。
...
- 桌面侧栏支持 264px 展开与 80px 图标模式；移动端用原导航按钮。右上角账号菜单支持 Escape 返回焦点，左下角另有账号设置入口。
...
侧面板标题栏保持固定高度，长标题可收缩换行，正文独立滚动；关闭后重新打开恢复普通尺寸。手机表单输入与文本框使用16px，市场筛选为两列，避免英文标签与数量挤压。桌面输入同为16px；刷新期间和客户端读取失败时禁止配置写入，新操作清除旧反馈。以上为实现规范，本轮尚未进行实际桌面/手机视觉验收。

### 桌面工作台视觉精修
默认窗口1180×800；侧栏216px，概览在900px以上保持账号与客户端双栏。顶部与插画区域收紧，面板使用细边框；网关以薄荷色分组，与本地安装清单区分。中文字体增加 Noto Sans SC 回退。
