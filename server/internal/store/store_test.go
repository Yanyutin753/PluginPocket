...
func TestBillingRoleMultiplierAppliesAtReserve(t *testing.T) {
	s := testStore(t)
	u, w, tok := fixture(t, s)
	ctx := context.Background()
	if _, e := s.Pool.Exec(ctx, "UPDATE users SET billing_role='vip' WHERE id=$1", u); e != nil {
		t.Fatal(e)
	}
	var toolID int64
	if e := s.Pool.QueryRow(ctx, "INSERT INTO tools(key,name,kind,cost) VALUES('vip_tool','VT','http',5) RETURNING id").Scan(&toolID); e != nil {
		t.Fatal(e)
	}
	...
	var role string
	if e = s.Pool.QueryRow(ctx, "SELECT balance FROM wallets WHERE id=$1", w).Scan(&balance); e != nil {
		t.Fatal(e)
	}
	if balance != 7 {
		t.Fatalf("vip 0.5x of price 5 must charge 3, balance=%d", balance)
	}
	if e = s.Pool.QueryRow(ctx, "SELECT cost,billing_role,multiplier_bp FROM usage_logs WHERE id=$1", call.ID).Scan(&cost, &role, &bp); e != nil {
		t.Fatal(e)
	}
	if cost != 3 || role != "vip" || bp != 5000 {
		t.Fatalf("usage row cost=%d role=%s bp=%d", cost, role, bp)
	}
	if _, e = s.Pool.Exec(ctx, "UPDATE billing_roles SET multiplier_bp=0 WHERE name='vip'"); e != nil {
		t.Fatal(e)
	}
	...
	if e = s.Pool.QueryRow(ctx, "SELECT cost FROM usage_logs WHERE id=$1", free.ID).Scan(&cost); e != nil {
		t.Fatal(e)
	}
	if cost != 0 {
...
func TestToolRoleGating(t *testing.T) {
	s := testStore(t)
	u, w, tok := fixture(t, s)
	ctx := context.Background()
	var toolID int64
	if e := s.Pool.QueryRow(ctx, "INSERT INTO tools(key,name,kind,cost,allowed_roles) VALUES('gated','G','http',5,'[\"vip\"]') RETURNING id").Scan(&toolID); e != nil {
		t.Fatal(e)
	}
	...
	if _, e = s.Pool.Exec(ctx, "UPDATE users SET billing_role='default' WHERE id=$1", u); e != nil {
		t.Fatal(e)
	}
	if _, e = s.Pool.Exec(ctx, "UPDATE tools SET allowed_roles='[]' WHERE id=$1", toolID); e != nil {
		t.Fatal(e)
	}
