      ...
      <main className="mx-auto flex max-w-5xl flex-col gap-10 px-5 py-10 sm:px-8 sm:py-14">
        ...
        >
          ...
          <dl className="grid grid-cols-2 gap-6 px-6 py-5 text-sm sm:px-8">
            <div className="flex flex-col gap-2">
              <dt className="text-muted-foreground">{t('服务版本')}</dt>
              <dd className="font-mono break-all">
                {connected ? health.data.version : '—'}
              </dd>
            </div>
            <div className="flex flex-col gap-2">
              <dt className="text-muted-foreground">{t('当前阶段')}</dt>
              <dd>{t('服务健康检查')}</dd>
            </div>
          </dl>
        </section>
        ...
        >
          <div className="flex flex-col gap-2">
            ...
            <p className="text-sm leading-6 text-muted-foreground">
              {t('先确认服务连接，再进入控制台创建令牌并配置客户端。')}
            </p>
          </div>
          <ol className="grid gap-6 sm:grid-cols-3 sm:gap-8">
            <li className="flex flex-col gap-2">
              <h3 className="font-medium">{t('网页开通')}</h3>
              <p className="text-sm leading-6 text-muted-foreground">
                {t('注册账号，创建专属网关令牌。')}
              </p>
            </li>
            <li className="flex flex-col gap-2">
              <h3 className="font-medium">{t('本地配置')}</h3>
              <p className="text-sm leading-6 text-muted-foreground">
                {t('通过 CLI 登录，一次配置多个 AI 客户端。')}
              </p>
            </li>
            <li className="flex flex-col gap-2">
              <h3 className="font-medium">{t('工具就绪')}</h3>
              <p className="text-sm leading-6 text-muted-foreground">
                {t('使用预设 MCP 工具，统一查看用量与额度。')}
              </p>
            </li>
          </ol>
        </section>
