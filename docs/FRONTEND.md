...
SidePanel统一支持全屏/还原，切换尺寸保持内容挂载以保留草稿，关闭后再次打开恢复普通尺寸；Escape在全屏时先还原，普通尺寸仍遵守locked关闭保护与原有焦点返回。
...
执行：`pnpm --dir web test` → `pnpm --dir web typecheck` → `make check`。CI 保留 harness 日志；DOM 测试不作为视觉通过证据。历史工坊重设计曾单独授权浏览器自动化与保留原画的本地抠图，该授权仅对应当轮执行记录；后续任务遵循 AGENTS.md 的不使用浏览器自动化约定。具体 RED/GREEN 工作流见 HARNESS.md。
...
2026-09-10 历史工坊重设计覆盖 18 条路由、浅深两主题、390/1440/2560px 宽度，共 108 张主截图及 20 张手机底部截图。人工审阅与行为测试证据、未完成的最终检查统一记入 docs/superpowers/plans/2026-09-10-workshop-ui.md。
...
用量列表“查看详情”按需请求独立详情接口并打开 SidePanel；输入输出作为原始文本显示（不得 JSON.parse 后丢失大整数精度），可键盘滚动。区分未记录、截断、加载和失败重试，Escape 返回入口。
