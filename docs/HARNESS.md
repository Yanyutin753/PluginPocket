...
- CI（`ci.yml`）按检测类别并行拆分：Lint（Biome/TS/golangci-lint/rustfmt+clippy，含桌面）、SQL 双方言迁移轨道（SQLite track，纯 Go 零外部依赖）、Server（真实 PostgreSQL+Redis，-race 全量）、Web 组件测试、Desktop（Linux/macOS/Windows 三平台矩阵，UI 测试 + 无 tauri 与默认 feature 两遍 cargo 测试；Windows runner 无 make，直接展开 `test-desktop` 底层命令）、CLI 三平台矩阵（Linux/macOS/Windows fmt+clippy+test+build）、容器镜像构建、端到端产品旅程（release 预检 + `make test-e2e`/test-process/test-dev/integration，仅 Linux，上传 `e2e-evidence` 产物含旅程日志与 deb）。各 job 合计覆盖 `make check` 的全部步骤；`make check` 新增检查时同步补进对应 job。
